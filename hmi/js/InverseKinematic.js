...
var InverseKinematic = function(geometry, jointLimits) {
...
InverseKinematic.prototype = {
    calculateAngles: function(x, y, z, a, b, c, angles) {
        ...
        var J2J4_length_x_y = this.length2(this.geometry[2][0] + this.geometry[3][0], this.geometry[2][1] + this.geometry[3][1])
        ...
        var J4J5_J4J3_normal_vector = this.cross(J4J5_vector, J4J3_vector);

        //addVectorArrow('normal J4', J[4], J4J5_J4J3_normal_vector)

        var XZ_parallel_aligned_vector = [10 * Math.cos(A[0] + Math.PI / 2),
            0, -10 * Math.sin(A[0] + Math.PI / 2)
        ]

        //addVectorArrow('normal J4 Y_vector', J[4], XZ_parallel_aligned_vector)

        var reference = this.cross(XZ_parallel_aligned_vector, J4J3_vector);

        //addVectorArrow('normal J4 Y_vectors', J[4], reference)

        var tmp = this.dot(reference, J4J5_J4J3_normal_vector);
        ...
        var reference_vector = this.cross(J4J3_vector, J4J5_J4J3_normal_vector)
        ...
        var reference_vector3 = this.cross(J4J5_vector, [0, 10, 0])

        var reference_vector2 = this.cross(J4J5_vector, reference_vector3)
        ...
        //addVectorArrow('2', J[5], J4J5_J4J3_normal_vector, 0x00ff00)
    ...
    calculateTCP: function(A0, A1, A2, A3, A4, A5, jointsResult) {
        var joints = [
            [],
            [],
            [],
    ...
    calculateCoordinates: function(A0, A1, A2, A3, A4, A5, jointsResult) {
        var a = Math.cos(A0);
        var b = Math.sin(A0);
        ...
        var A = Math.cos(A5);
        var B = Math.sin(A5);
        var C = 0; //this.geometry[5][0]
        var D = 0; //this.geometry[5][1]
        var E = 0; //this.geometry[5][2]
        ...
        function rotate(value, angle, max) {
            var min = (max - 2 * Math.PI)
            var newVal = (value + angle)
    ...
    cross: function(vectorA, vectorB, result) {
        return result = [
    ...
    angleBetween: function(vectorA, vectorB, referenceVector) {

        //angle = atan2(norm(cross(a, b)), dot(a, b))

        var norm = this.length3(this.cross(vectorA, vectorB));

        var angle = Math.atan2(norm, (vectorB[0] * vectorA[0] + vectorB[1] * vectorA[1] + vectorB[2] * vectorA[2]));
